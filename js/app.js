...
function updateBatteryUI(battery) {
  const update = () => {
    ...
    // Mobile context bar update
    const mEl = document.getElementById('mBatteryVal');
    if (mEl) mEl.textContent = `${charging} ${level}%`;
  };
...
function updateNetworkInfo() {
  ...
  const mEl = document.getElementById('mNetworkVal');
  if (!el && !mEl) return;

  if (!navigator.onLine) {
    if (el) el.innerHTML = '<span style="color: var(--color-error-400);">Offline</span>';
    if (mEl) mEl.textContent = 'Offline';
    return;
  }
  ...
  if (el) el.innerHTML = `<span style="color: var(--color-success-400);">${type} ${speed}</span>`;
  if (mEl) mEl.textContent = type;
}
...
function updateTime() {
  ...
  const mTimeElement = document.getElementById('mTimeVal');
  if (timeElement || mTimeElement) {
    const now = new Date();
    ...
    if (mTimeElement) {
      mTimeElement.textContent = now.toLocaleTimeString('en-US', {
        hour: '2-digit',
        minute: '2-digit'
      });
    }
