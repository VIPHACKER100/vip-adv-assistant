...
class VoiceManager {
    ...
    updateSpeakingUI(isSpeaking) {
        ...
        window.dispatchEvent(new CustomEvent('voiceSpeaking', {
            detail: { isSpeaking }
        }));
    }
    ...
    renderVoiceCategories(categories) {
        ...
        if (categories.other.length > 0) {
            html += this.renderVoiceSection('📢 Other Voices', categories.other, 'other');
        }
     ...
     */
    renderVoiceSection(title, voices, category) {
        const html = `
